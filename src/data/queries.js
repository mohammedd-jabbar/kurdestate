...
export const addListing = async (form) => {
  ...
  await addDoc(collection(db, "listings"), formDataCopy); // add the listing to the firestore
};

// Edit listing
export const editListing = async (form) => {
  let location = {}; // location google api
  ...
  if (form.images !== undefined) {
    ...
    const formDataCopy = {
      ...form.formDataEn,
      ...form.formDataKu, // copy the formData object to formDataCopy
      imgUrls: [imgUrls, form.formDataEn.imgUrls].flat(), // add the image urls that we uploaded to the firebase storage
      status: listingStatus,
      location, // and manually latitude and longitude
      timeStamp: serverTimestamp(), // add the timestamp when the listing was created
      userRef: auth.currentUser.uid, // add the user id to the listing
    ...
    const docRef = doc(db, "listings", form.listId);
    await updateDoc(docRef, formDataCopy); // add the listing to the firestore
  } else {
    const formDataCopy = {
      ...form.formDataEn,
      ...form.formDataKu, // copy the formData object to formDataCopy
